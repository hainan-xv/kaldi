...
cat $filelist | sed "s=^=$transpath/fisher_spa_tr/data/transcripts/=g" | sed "s=$=.tdf=g" > $tmpdir/tdf.flist
transfilelist=`cat $tmpdir/tdf.flist`

if [ $stage -le 1 ]; then # text
  cat $transfilelist | grep -v "file;unicode" | awk -F '\t' 'NF==13' | gzip > $tmpdir/all_text.gz
