...
if [ $stage -le 1 ]; then
  cat data/train_fisher/text data/train_callhome/text data/train_hub/text | awk '{for(i=2;i<=NF;i++) print $i}' | sort | grep -v "<" | grep . | uniq -c | sort -k1nr | awk '{print $2}' > $tmpdir/wordlist.txt
#  cat data/train_fisher/text data/train_callhome/text data/local/gigatmp/text data/train_hub/text | cut -f2- | sed "s= =\n=g" | sort | grep -v "<" | grep . | uniq -c | sort -k1nr | head -n 60000 | awk '{print $2}' > $tmpdir/wordlist.txt
