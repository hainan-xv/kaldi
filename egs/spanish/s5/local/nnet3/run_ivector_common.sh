...
if [ $stage -le 1 ]; then
  ...
  for train_set in train_hub train_hub_sp; do
    ...
    mfccdir=mfcc_perturbed
    steps/make_mfcc.sh --cmd "$train_cmd" --nj 50 --mfcc_config conf/mfcc.conf.hub \
      data/${train_set}_tmp exp/make_mfcc/${train_set}_tmp $mfccdir || exit 1;
    steps/compute_cmvn_stats.sh data/${train_set}_tmp exp/make_mfcc/${train_set}_tmp $mfccdir || exit1;
    utils/fix_data_dir.sh data/${train_set}_tmp
  ...
  utils/combine_data.sh data/train_sp data/train_callhome_sp data/train_fisher_sp data/train_hub_sp
fi
...
#  for dataset in $train_set $train_set_sp; do
  for dataset in train_fisher train_callhome train_fisher_sp train_callhome_sp; do
...
"| sort -k1,1 -u  > $data_dir/wav.scp_scaled || exit 1;
  ...
  for dataset in train_hub train_hub_sp; do
    data_dir=data/${dataset}_hires
    utils/copy_data_dir.sh data/$dataset $data_dir
...
"| sort -k1,1 -u  > $data_dir/wav.scp_scaled || exit 1;
    ...
    steps/make_mfcc.sh --nj 70 --mfcc-config conf/mfcc_hires.conf.hub \
      $data_dir exp/make_hires/$dataset $mfccdir
    steps/compute_cmvn_stats.sh $data_dir exp/make_hires/$dataset $mfccdir
    utils/fix_data_dir.sh $data_dir # remove segments with problems
  done

  utils/combine_data.sh data/train_sp_hires data/train_callhome_sp_hires data/train_fisher_sp_hires data/train_hub_sp_hires
  utils/combine_data.sh data/train_hires data/train_callhome_hires data/train_fisher_hires data/train_hub_hires

  for dataset in test_fisher test_callhome; do
    data_dir=data/${dataset}_hires
    utils/copy_data_dir.sh data/$dataset $data_dir
  ...
  utils/combine_data.sh data/test_hires data/test_callhome_hires data/test_fisher_hires
#  utils/combine_data.sh data/test_hires data/test_callhome_hires data/test_fisher_hires data/test_hub_hires
...
if [ $stage -le 5 ]; then
  ...
  steps/train_lda_mllt.sh --cmd "$train_cmd" --num-iters 13 \
    ...
    5000 120000 data/${train_set}_hires \
    data/lang exp/tri5a_ali exp/nnet3/tri5b
fi

if [ $stage -le 6 ]; then
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 --num-frames 700000 \
    data/${train_set_sp}_hires 512 exp/nnet3/tri5b exp/nnet3/diag_ubm
fi
...
if [ $stage -le 8 ]; then
    ...
    for data_set in test test_fisher test_callhome; do
