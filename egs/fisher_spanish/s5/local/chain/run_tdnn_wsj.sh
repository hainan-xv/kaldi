...
common_egs_dir=exp/chainfrom_wsj/tdnn1f_sp_backstitc0.2_4_680/egs
reporting_email=
...
dir=exp/chain${nnet3_affix}/tdnn${affix}_sp_backstitc0.2_4_680
...
if [ $stage -le 15 ]; then
  ...
  relu-renorm-layer name=tdnn3 dim=$dim input=Append(-1,0,1)
  relu-renorm-layer name=tdnn4 dim=$dim input=Append(-3,0,3)
  relu-renorm-layer name=tdnn5 dim=$dim input=Append(-3,0,3)
  relu-renorm-layer name=tdnn6 dim=$dim input=Append(-6,-3,0)
...
if [ $stage -le 16 ]; then
  ...
  steps/nnet3/chain/train.py --stage=$train_stage \
    ...
    --trainer.optimization.final-effective-lrate=0.002 \
    --trainer.optimization.shrink-value=1.0 \
    --trainer.optimization.proportional-shrink=10.0 \
