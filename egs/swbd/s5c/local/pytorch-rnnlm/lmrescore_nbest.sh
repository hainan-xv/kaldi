...
# LM rescoring with PyTorch RNNLM.
...
for f in $rnndir/model.pt $data/feats.scp $indir/lat.1.gz; do
...
if [ $stage -le 6 ]; then
  ...
  $cmd JOB=1:$nj $dir/log/rnnlm_compute_scores.JOB.log \
    local/pytorch-rnnlm/compute_sentence_scores.sh $rnndir $adir.JOB/temp \
