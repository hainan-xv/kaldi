...
if [ $stage -le 1 ]; then
  for datadir in train test dev; do
...
if [ $stage -le 2 ]; then
  ...
  steps/align_fmllr.sh --nj 40 --cmd "$train_cmd" \
    data/train data/lang exp/tri3 exp/nnet3/tri3_ali
fi
...
if [ $stage -le 3 ]; then
  ...
  steps/train_lda_mllt.sh --cmd "$train_cmd" --num-iters 13 \
    ...
    5000 10000 data/train_hires data/lang \
     exp/nnet3/tri3_ali exp/nnet3/tri4
fi
...
if [ $stage -le 4 ]; then
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 \
     --num-frames 400000 data/train_hires 256 exp/nnet3/tri4 exp/nnet3/diag_ubm
fi
...
false &&  steps/online/nnet2/copy_data_dir.sh --utts-per-spk-max 2 data/train_hires \
    data/train_hires_max2

  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj 30 \
    data/train_hires exp/nnet3/extractor exp/nnet3/ivectors_train || exit 1;
fi
