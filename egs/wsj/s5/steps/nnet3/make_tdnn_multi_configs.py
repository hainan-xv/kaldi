...
total_num_params = 0
for l in range(1, num_hidden_layers):  # we deal with the last layer separately
...
#        total_num_params = total_num_params + nonlin_output_dim * i # only count them once, in the last layer file
        nnn = nnn + 1
...
for l in range(num_hidden_layers, num_hidden_layers + 1):
    f = open(args.config_dir + "/layer{0}.config".format(l), "w")
    print('# Config file for layer {0} of the network'.format(l), file=f)
