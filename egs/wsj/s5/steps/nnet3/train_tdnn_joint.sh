...
if [ $stage -le -5 ]; then
  ...
  python steps/nnet3/make_tdnn_joint_configs.py  \
    ...
    --num-leaves-total $total_leaves \
    --num-targets $num_virtual \
...
if [ $stage -le -4 ] && [ -z "$egs_dir" ]; then
  ...
  steps/nnet3/get_egs.sh $egs_opts "${extra_opts[@]}" \
      ...
      $data $virtualdir $dir/egs || exit 1;
fi
...
if [ $stage -le -2 ]; then
  ...
  echo "  ... using priors^$presoftmax_prior_scale_power and rescaling to average 1"
    ...
    $cmd JOB=1:$nj $dir/log/acc_pdf.JOB.log \
       ali-to-post "ark:gunzip -c ${virtualdir}/ali.JOB.gz|" ark:- \| \
       post-to-tacc --per-pdf=true  ${virtualdir}/final.mdl ark:- $dir/pdf_counts.JOB || exit 1;
...
if [ $stage -le -1 ]; then
  ...
  $cmd $dir/log/init_mdl.log \
      nnet3-am-init $virtualdir/final.mdl $dir/0.raw - \| \
      nnet3-am-train-transitions - "ark:gunzip -c $virtualdir/ali.*.gz|" $dir/0.mdl || exit 1;
fi
...
# TODO(hxu)
    if [ $x -gt 0 ]; then
      $cmd $dir/log/progress.$x.log \
        ...
        "nnet3-am-copy --raw=true $dir/$[$x-1].mdl - |" "nnet3-am-copy --raw=true $dir/$x.mdl - |" \
        "ark:nnet3-merge-egs ark:$cur_egs_dir/train_diagnostic.egs ark:-|" '&&' \
...
#'''
  $cmd JOB=1:$num_jobs_compute_prior $prior_queue_opt $dir/log/get_post.$x.JOB.log \
    ...
    nnet3-compute-from-egs $prior_gpu_opt --apply-exp=true \
      "nnet3-am-copy --raw=true $dir/combined.mdl -|" ark:- ark:$dir/egs_tmp
#  ''' 2>/dev/null   
#      \| \

    $cmd JOB=1:$num_jobs_compute_prior $prior_queue_opt $dir/log/get_post.$x.JOB.log \
    matrix-sum-rows ark:$dir/egs_tmp ark:- \| vector-sum ark:- $dir/post.$x.JOB.vec || exit 1;
