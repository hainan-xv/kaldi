...
# Check some files.
for f in $data/feats.scp $lang/L.fst ${alidir}_0/ali.1.gz ${alidir}_0/final.mdl ${alidir}_0/tree; do
...
for i in `seq 0 $[$num_outputs-1]`; do
  ...
  [ "${num_leaves[$i]}" -eq "0" ] && echo "\$num_leaves is 0" && exit 1
done

nj=`cat ${alidir}_0/num_jobs` || exit 1;  # number of jobs in alignment dir...
...
target_string="${num_leaves[0]}"

for i in `seq 1 $[$num_outputs-1] `; do
  target_string="$target_string,${num_leaves[$i]}"
done
target_string="$target_string"
echo target_string is $target_string
...
#    --last-factor $last_factor \
  ...
  python steps/nnet3/make_tdnn_multi_configs.py  \
    ...
    --num-targets-multi  $target_string  \
   $dir/configs || exit 1;
...
if [ $stage -le -4 ] && [ -z "$egs_dir" ]; then
  ...
  steps/nnet3/get_egs_multi.sh $egs_opts "${extra_opts[@]}" \
...
if [ $stage -le -3 ]; then
  ...
  $cmd JOB=1:$num_lda_jobs $dir/log/get_lda_stats.JOB.log \
      nnet3-acc-lda-stats-multi --rand-prune=$rand_prune \
      --num-outputs=$num_outputs \
        $dir/init.raw "ark:$egs_dir/egs.JOB.ark" $dir/JOB.lda_stats || exit 1;

  for i in `seq 0 $[$num_outputs-1] `; do
    all_lda_accs=$(for n in $(seq $num_lda_jobs); do echo $dir/$n.lda_stats-$i; done)
    $cmd $dir/log/sum_transform_stats_$i.log \
      sum-lda-accs $dir/lda_stats_$i $all_lda_accs || exit 1;
    ...
    $cmd $dir/log/get_transform_$i.log \
       nnet-get-feature-transform $lda_opts $dir/lda_$i.mat $dir/lda_stats_$i || exit 1;

    ln -sf ../lda_$i.mat $dir/configs/lda_$i.mat
  done
  ln -sf ../lda_0.mat $dir/configs/lda.mat  # TODO(hxu) maybe should sum over all lda stat?
fi
...
if [ $stage -le -2 ]; then
  ...
  for i in `seq 0 $[$num_outputs-1] `; do
    ...
    $cmd JOB=1:$nj $dir/log/acc_pdf.JOB.log \
       ali-to-post "ark:gunzip -c ${alidir}_$i/ali.JOB.gz|" ark:- \| \
       post-to-tacc --per-pdf=true  ${alidir}_$i/final.mdl ark:- $dir/pdf_counts_$i.JOB || exit 1;
    $cmd $dir/log/sum_pdf_counts.log \
         vector-sum --binary=false $dir/pdf_counts_$i.* $dir/pdf_counts_$i || exit 1;
    rm $dir/pdf_counts_$i.*
    ...
    awk -v power=$presoftmax_prior_scale_power -v smooth=0.01 \
       ...
       $dir/pdf_counts_$i > $dir/presoftmax_prior_scale.vec$i
    ln -sf ../presoftmax_prior_scale.vec$i $dir/configs/presoftmax_prior_scale.vec$i
  done
fi
...
if [ $stage -le -1 ]; then
  ...
  input_string1="${alidir}_0/final.mdl"
  input_string2="\"ark:gunzip -c ${alidir}_0/ali.*.gz|\""

  for i in `seq 1 $[$num_outputs-1] `; do
    input_string1="$input_string1 ${alidir}_$i/final.mdl"
    input_string2="$input_string2 \"ark:gunzip -c ${alidir}_$i/ali.*.gz|\""
  done
  ...
  $cmd $dir/log/init_mdl.log \
      nnet3-am-init-multi --num-outputs=$num_outputs $input_string1 $dir/0.raw - \| \
      nnet3-am-train-transitions-multi - $input_string2 $dir/0.mdl || exit 1;

  nnet3-am-copy-multi --binary=false $dir/0.mdl $dir/0.mdl.txt
  nnet3-am-copy --binary=false $baseline_dir/final.mdl $dir/0.mdl.reference.txt # TODO(hxu) maybe make it nnet3-am-copy

  for i in `seq 0 $[$num_outputs-1] `; do
    cat $dir/configs/presoftmax_prior_scale.vec$i | sed "s=\[==g" | sed "s=\]==g" > $dir/scale.vec$i
    tail -n $num_outputs $dir/0.mdl.txt | sed -n $[$i+1]p | sed "s=^.*\[==g" | sed "s=\].*$==g" > $dir/prior.vec$i
  done
  ...
  cp $dir/0.mdl $dir/$jump.mdl
  cp $dir/0.mdl $dir/$[$jump-1].mdl

  if [ $stage -lt $[$jump] ]; then
    stage=$[$jump]
  fi
fi
...
false && (! [ $num_iters -gt $[$finish_add_layers_iter+2] ] \
...
'''    if [ ! -z "${realign_this_iter[$x]}" ]; then
      ...
      # we are using different random subsets of it.
...
''' 2>/dev/null
    ...
    $cmd $dir/log/compute_prob_valid.$x.log \
      nnet3-compute-prob "nnet3-am-copy-multi --raw=true $dir/$x.mdl - |" \
...
# TODO(hxu)
    if [ $x -gt 0 ]; then
      $cmd $dir/log/progress.$x.log \
        nnet3-show-progress-multi --use-gpu=no --num-outputs=$num_outputs \
        "nnet3-am-copy-multi --raw=true $dir/$[$x-1].mdl - |" "nnet3-am-copy-multi --raw=true $dir/$x.mdl - |" \
        "ark:nnet3-merge-egs ark:$cur_egs_dir/train_diagnostic.egs ark:-|" '&&' \
        nnet3-info "nnet3-am-copy-multi --raw=true $dir/$x.mdl - |" &
    fi
    ...
    if [ $x -gt 0 ] && \
      ...
      raw="nnet3-am-copy-multi --raw=true --learning-rate=$this_learning_rate $dir/$x.mdl - | nnet3-init --srand=$x - $config - |"
    else
      do_average=true
      if [ $x -eq 0 ]; then do_average=false; fi # on iteration 0, pick the best, don't average.
      raw="nnet3-am-copy-multi --raw=true --learning-rate=$this_learning_rate $dir/$x.mdl -|"
    fi
    if $do_average; then
    ...
    if $do_average; then
      ...
      $cmd $dir/log/average.$x.log \
        ...
        nnet3-am-copy-multi --set-raw-nnet=- $dir/$x.mdl $dir/$[$x+1].mdl || exit 1;
    else
      ...
      $cmd $dir/log/select.$x.log \
        nnet3-am-copy-multi --set-raw-nnet=$dir/$[$x+1].$n.raw  $dir/$x.mdl $dir/$[$x+1].mdl || exit 1;
    fi
...
if [ $stage -le $num_iters ]; then
  ...
  for n in $(seq 0 $[num_iters_combine-1]); do
    ...
    nnets_list[$n]="nnet3-am-copy-multi --raw=true $mdl -|";
  done
  ...
  $cmd $combine_queue_opt $dir/log/combine.log \
    nnet3-combine-multi --num-iters=40 \
       --num-outputs=$num_outputs \
       --enforce-sum-to-one=true --enforce-positive-weights=true \
       --verbose=3 "${nnets_list[@]}" "ark:nnet3-merge-egs --minibatch-size=1024 ark:$cur_egs_dir/combine.egs ark:-|" \
    "|nnet3-am-copy-multi --set-raw-nnet=- $dir/$num_iters.mdl $dir/combined.mdl" || cp $dir/${num_iters}.mdl $dir/combined.mdl

  # Compute the probability of the final, combined model with
  # the same subset we used for the previous compute_probs, as the
  # different subsets will lead to different probs.
  $cmd $dir/log/compute_prob_valid.final.log \
    nnet3-compute-prob "nnet3-am-copy-multi --raw=true $dir/combined.mdl -|" \
    "ark:nnet3-merge-egs ark:$cur_egs_dir/valid_diagnostic.egs ark:- |" &
...
if [ $stage -le $[$num_iters+1] ]; then
  echo "Getting average posterior for purposes of adjusting the priors."
  echo number archves $num_archives, numjobs $num_jobs_compute_prior
  # Note: this just uses CPUs, using a smallish subset of data.
  if [ $num_jobs_compute_prior -gt $num_archives ]; then egs_part=1;
  else egs_part=JOB; fi
  rm $dir/post.$x.*.vec 2>/dev/null
#'''
  echo egs_part $egs_part
  ...
  for i in `seq 0 $[${num_outputs}-1]`; do
    $cmd JOB=1:$num_jobs_compute_prior $prior_queue_opt $dir/log/get_post.$x.JOB.log \
      ...
      matrix-sum-rows ark:- ark:- \| vector-sum ark:- $dir/post.$x.JOB.vec$i || exit 1;

    sleep 3;  # make sure there is time for $dir/post.$x.*.vec to appear.

    $cmd $dir/log/vector_sum.$x.$i.log \
     vector-sum $dir/post.$x.*.vec$i $dir/post.$x.vec$i || exit 1;
  ...
  $cmd $dir/log/adjust_priors.final.log \
    nnet3-am-adjust-priors-multi $dir/combined.mdl $dir/post.$x.vec $dir/final.mdl || exit 1;
fi
