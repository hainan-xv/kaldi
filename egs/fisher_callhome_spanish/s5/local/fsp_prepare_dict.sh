...
datadir2=`pwd`/data/local/data/callhome_train_all
mkdir -p $dir
local=`pwd`/local
utils=`pwd`/utils
...
if [ $stage -le 0 ]; then
  cat $datadir/text $datadir2/text | sed 's:[0-9][0-9]\S*::g' | sed 's:[\.,\?\<]::g' | tr " " "\n" | grep -v "î" | sort | uniq | awk '{if (NF > 0){ print; }}' > $tmpdir/uniquewords
