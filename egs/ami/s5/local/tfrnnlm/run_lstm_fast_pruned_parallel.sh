...
dir=data/tensorflow_lstm_fast_fix_pruned_parallel
mkdir -p $dir
...
# the following script uses TensorFlow. You could use tools/extras/install_tensorflow_py.sh to install it
  $cuda_cmd $dir/train_rnnlm.log utils/parallel/limit_num_gpus.sh \
    python steps/tfrnnlm/train_lstm_fast_fix.py --data-path=$dir --save-path=$dir/rnnlm --vocab-path=$dir/wordlist.rnn.final
fi
...
# pruned lattice rescoring
  steps/tfrnnlm/lmrescore_rnnlm_lat_pruned_parallel.sh \
    --cmd "$tfrnnlm_cmd --mem 4G" \
    --weight $weight --max-ngram-order $ngram_order \
    data/lang_$LM $dir \
    data/$mic/${decode_set}_hires ${decode_dir} \
    ${decode_dir}_tfrnnlm_lat_pruned_parallel_${ngram_order}gram  &
...
#      data/lang_$LM $dir \
#      data/$mic/${decode_set}_hires ${decode_dir} \
#      ${decode_dir}_lat_lstm_fast_fix_${ngram_order}gram_unpruned  &
