...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    const char *usage =
        "Train decision tree\n"
        "Usage:  build-tree-random [options] <tree-stats-in> <roots-file> <questions-file> <topo-file> <tree-out-prefix>\n"
        "e.g.: \n"
    ...
    po.Register("num-trees", &num_trees, "Number of random trees to build");
    po.Register("rand-prob", &rand_prob, "Probability in random selection");
    ...
    for (int i = 0; i < num_trees; i++) {
      ...
      std::stringstream ss;
      ss << i;
      WriteKaldiObject(ctx_dep, tree_out_filename + "-" + ss.str(), binary);
