...
int main(int argc, char *argv[]) {
  try {
    ...
    TfRnnlmDeterministicFstParallel* lm_to_add_orig = 
      new TfRnnlmDeterministicFstParallel(max_ngram_order, &rnnlm);

    for (; !compact_lattice_reader.Done(); compact_lattice_reader.Next()) {
      fst::DeterministicOnDemandFstParallel<StdArc> *lm_to_add =
         new fst::ScaleDeterministicOnDemandFstParallel(lm_scale, lm_to_add_orig);
      ...
      fst::ComposeDeterministicOnDemandFstParallel<StdArc> combined_lms(
          lm_to_subtract_det_scale, lm_to_add);

      // Composes lattice with language model.
      CompactLattice composed_clat;
      ComposeCompactLatticePrunedParallel(compose_opts, clat,
