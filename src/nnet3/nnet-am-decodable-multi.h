...
struct NnetMultiComputationOptions {
  int32 num_outputs;
  BaseFloat exp_weight;
...
class NnetDecodableMultiBase {
  ...
  */
  NnetDecodableMultiBase(const NnetMultiComputationOptions &opts,
                         const Nnet &nnet,
 ...
 private:
  ...
  NnetMultiComputationOptions opts_;
  const Nnet &nnet_;
  ...
  // ran the computation.
  vector<Matrix<BaseFloat> > current_log_post_vec_;
...
class DecodableAmNnetMulti: public DecodableInterface,
                            private NnetDecodableMultiBase {
 public:
  /**
     This constructor takes features as input, and you can either supply a
     single iVector input, estimated in batch-mode ('ivector'), or 'online'
     ...
     @param [in] online_ivector_period If you are using iVectors estimated 'online'
                        ...
                        (in frames) with which the iVectors are estimated.
  */
  DecodableAmNnetMulti(const NnetMultiComputationOptions &opts,
