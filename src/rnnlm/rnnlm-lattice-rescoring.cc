...
bool KaldiRnnlmDeterministicFst::GetArc(StateId s, Label ilabel,
  ...
  if (iter != unigrams_.end()) {
    double tmp = logprob;
//    logprob = logprob * 0.9 + log(iter->second) * 0.1;
//    KALDI_LOG << "after: " << logprob << "\n";
//    std::cout << iter->second << " and " << ori_unigrams_[ilabel] << std::endl;
    double weight = 1.0;

    double diff = (iter->second + 0.0000001) / (ori_unigrams_[ilabel] + 0.0000001);
//    double diff = (iter->second + 0.000001) / (ori_unigrams_[ilabel] + 0.000001);
    diff = diff / 2 + 0.5;
    diff = std::max(diff, 1.0);
    diff = std::min(diff, 2.0);
    diff = log(diff);
    logprob += diff;
    KALDI_LOG << "weight is " << weight << ", before: " << tmp << ", after: " << logprob << "\n";
  }
