...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    std::string model_in_filename = po.GetArg(i++),
        trans_file = po.GetArg(i++),
        mapping_file = po.GetArg(i++),
        fst_in_str = po.GetArg(i++),
        feature_rspecifier = po.GetArg(i++),
        lattice_wspecifier = po.GetArg(i++),
        words_wspecifier = po.GetOptArg(i++),
        alignment_wspecifier = po.GetOptArg(i++);
    ...
    ReadKaldiObject(trans_file, &trans_model);
