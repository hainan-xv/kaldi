...
class NnetLdaStatsAccumulatorMulti {
 public:
  NnetLdaStatsAccumulatorMulti(BaseFloat rand_prune,
                               const Nnet &nnet, int num_outputs):
      rand_prune_(rand_prune), nnet_(nnet), compiler_(nnet),
      num_outputs_(num_outputs) {
      lda_stats_.resize(num_outputs, NULL);
  }
  ~NnetLdaStatsAccumulatorMulti() {
    for (int i = 0; i < lda_stats_.size(); i++) {
      delete lda_stats_[i];
    }
  }
  ...
  void AccStats(const NnetExample &eg) {
    ...
    for (int i = 0; i < num_outputs_; i++) {
      ...
      os << "output" << i;
      const CuMatrixBase<BaseFloat> &nnet_output = computer.GetOutput(os.str());
      AccStatsFromOutput(eg, nnet_output, i);
    }
  }

  void WriteStats(const std::string &stats_wxfilename, bool binary) {
    for (int i = 0; i < num_outputs_; i++) {
      if (lda_stats_[i]->TotCount() == 0) {
        KALDI_ERR << "Accumulated no stats.";
      } else {
        std::ostringstream os;
        os << "-" << i;
        WriteKaldiObject(*lda_stats_[i], stats_wxfilename + os.str(), binary);
        KALDI_LOG << "Accumulated stats, soft frame count = "
                  << lda_stats_[i]->TotCount() << ".  Wrote to "
                  << stats_wxfilename + os.str();
      }
    }
  }
 private:
  void AccStatsFromOutput(const NnetExample &eg,
    ...
    if (lda_stats_[output_index] == NULL) {
      lda_stats_[output_index] = new LdaEstimate();
    }
    ...
    if (lda_stats_[output_index]->Dim() == 0)
      lda_stats_[output_index]->Init(num_pdfs, nnet_output.NumCols());
    ...
    for (int32 r = 0; r < num_rows; r++) {
      ...
      for (; post_data != post_end; ++post_data) {
        ...
        if (pruned_weight != 0.0)
          lda_stats_[output_index]->Accumulate(row, pdf, pruned_weight);
      }
    }
  }
...
int main(int argc, char *argv[]) {
  try {
    ...
    po.Register("num-outputs", &num_outputs,
                "Number of outputs");
    ...
    NnetLdaStatsAccumulatorMulti accumulator(rand_prune, nnet, num_outputs);
