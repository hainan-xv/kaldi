...
int main(int argc, char *argv[]) {
  try {
    ...
    po.Register("num-samples", &num_samples, "number of samples per minibatch");
    ...
    std::string unigram_file        = po.GetArg(2);
    ...
    rnnlm::ReadUnigram(unigram_file, &unigram);

    vector<double> cdf(unigram.size() + 1);
    cdf[0] = 0;
    for (int i = 1; i < cdf.size(); i++) {
      cdf[i] = cdf[i - 1] + unigram[i - 1];
    }


    std::vector<NnetExampleWriter*> example_writers(num_outputs);
    for (int32 i = 0; i < num_outputs; i++)
      example_writers[i] = new NnetExampleWriter(po.GetArg(i+3));
